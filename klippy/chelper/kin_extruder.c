...
struct extruder_stepper {
    ...
    double ka, kb, alpha, beta;
};

static double
extruder_calc_position(struct stepper_kinematics *sk, struct move *m, double move_time)
{
    ...
    if (hst)
    {
        ...
        double dm = pa_range_integrate(m, move_time, es->pressure_advance, hst) * es->inv_half_smooth_time2;
        es->alpha = m->start_pos.z + m->axes_r.z*dm;
        es->beta = m->start_pos.x + m->axes_r.x*dm;
    }
    else
    {
    ...
    //errorf("--> kin_extruder: alpha = %lf, beta = %lf, s_vel = %lf, s_pos = %lf, acc = %lf, s_time = %lf, move_time = %lf",
    //        es->alpha, es->beta, m->start_v, m->print_time, m->half_accel, m->move_t, move_time);
    return es->ka*es->alpha + es->kb*es->beta;
}
...
static void
extruder_stepper_correction(struct stepper_kinematics *sk)
{
    struct extruder_stepper *es = container_of(sk, struct extruder_stepper, sk);
    sk->commanded_pos = es->ka*es->alpha + es->kb*es->beta;
}

struct stepper_kinematics * __visible
extruder_stepper_alloc(double ka, double kb)
{
    struct extruder_stepper *es = malloc(sizeof(*es));
    memset(es, 0, sizeof(*es));
    es->sk.calc_position_cb = extruder_calc_position;
    es->sk.active_flags = AF_X | AF_Z;
    es->ka = ka;
    es->kb = kb;
    es->alpha = 0;
    es->beta = 0;
    es->sk.post_cb = extruder_stepper_correction;
    return &es->sk;
}
